...
export class TaskCreateForm extends React.PureComponent {
  ...
  goingBack = (e) => {
    e.preventDefault();
    this.props.history.push('/taskkeeper');
  }

  render() {
    const { taskcreateform } = this.props.task;
    if (taskcreateform) {
      return <Redirect to='/taskkeeper' />;
    }
    return (
      <div key="taskcreateform" className="app__taskcreateform app__route">
        <div className="app__container">
          <Form className="app__taskcreateform__form" onSubmit={this.handleSubmit} >
            ...
            <Button type="button" onClick={this.goingBack}>Back</Button>
          </Form>
        </div>
      </div>
    );
